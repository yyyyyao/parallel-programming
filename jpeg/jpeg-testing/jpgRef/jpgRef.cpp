...
int main(int argc, char* argv[])
{
  ...
  printf("quant table Y\n");
  for(int i = 0; i < 8; i++) {
    for(int j = 0; j < 8; j++) {
      printf("%d ", quantTableY[i * 8 + j]);
    }
    printf("\n");
  }
  printf("quant table CbCr\n");
  for(int i = 0; i < 8; i++) {
    for(int j = 0; j < 8; j++) {
      printf("%d ", quantTableCbCr[i * 8 + j]);
    }
    printf("\n");
  }
  ...
  cv::Mat reverseBlockCr(halfHeight, halfWidth, CV_8UC1);
  unsigned char* reverseBlockCrPtr = (unsigned char*)reverseBlockCr.data;
  reverseBlockSplitting(blockCrPtr, reverseBlockCrPtr, halfWidth, halfHeight);
  ...
  for(int i = 0; i < height * width; i+=64) {
    centerYPtrBuf = centerYPtr + i;
    dctYPtrBuf = dctYPtr + i;
...
#define DCT_DEBUG 1
  ...
  for(int i = 0; i < halfHeight * halfWidth; i+=64) {
    centerCbPtrBuf = centerCbPtr + i;
    dctCbPtrBuf = dctCbPtr + i;
    twoDimDct(centerCbPtrBuf, dctCbPtrBuf);

    centerCrPtrBuf = centerCrPtr + i;
    dctCrPtrBuf = dctCrPtr + i;
  ...
  for(int i = 0; i < height * width; i+=64) {
    dctYPtrBuf = dctYPtr + i;
    zigzagYPtrBuf = zigzagYPtr + i;
    zigZagScan(dctYPtrBuf, zigzagYPtrBuf);

    printf("-----zigzag--------\n");
    for(int i = 0; i < 8; i++) {
      for(int j = 0; j < 8; j++) {
        printf(" %d", zigzagYPtrBuf[i * 8 + j]);
      }
      printf("\n");
    }
  ...
  for(int i = 0; i < halfHeight * halfWidth; i+=64) {
    dctCbPtrBuf = dctCbPtr + i;
    zigzagCbPtrBuf = zigzagCbPtr + i;
    zigZagScan(dctCbPtrBuf, zigzagCbPtrBuf);
    dctCrPtrBuf = dctCrPtr + i;
    zigzagCrPtrBuf = zigzagCrPtr + i;
    zigZagScan(dctCrPtrBuf, zigzagCrPtrBuf);

    printf("-----zigzagCb--------\n");
    for(int i = 0; i < 8; i++) {
      for(int j = 0; j < 8; j++) {
        printf(" %d", zigzagCbPtrBuf[i * 8 + j]);
      }
      printf("\n");
    }
    printf("-----zigzagCr--------\n");
  ...
  for(int i = 0; i < height * width; i+=64) {
    zigzagYPtrBuf = zigzagYPtr + i;
    quantYPtrBuf = quantYPtr + i;
    applyQuantTable(zigzagYPtrBuf, quantYPtrBuf, quantTableY);

    printf("-----quant & zizag Y:%d--------\n", i / 64);
    for(int k = 0; k < 8; k++) {
      for(int j = 0; j < 8; j++) {
        printf(" %d", quantYPtrBuf[k * 8 + j]);
      }
      printf("\n");
    }
  }
  ...
  for(int i = 0; i < halfHeight * halfWidth; i+=64) {
    zigzagCbPtrBuf = zigzagCbPtr + i;
    quantCbPtrBuf = quantCbPtr + i;
    applyQuantTable(zigzagCbPtrBuf, quantCbPtrBuf, quantTableCbCr);

    zigzagCrPtrBuf = zigzagCrPtr + i;
    quantCrPtrBuf = quantCrPtr + i;
    applyQuantTable(zigzagCrPtrBuf, quantCrPtrBuf, quantTableCbCr);

    printf("-----quant & zizag Cb:%d--------\n", i / 64);
    for(int k = 0; k < 8; k++) {
      for(int j = 0; j < 8; j++) {
        printf(" %d", quantCbPtrBuf[k * 8 + j]);
      }
      printf("\n");
    }
  ...
  int mcuX = width / 16;
  int mcuY = width / 16;
  int lineOfData = 64 * mcuX;
  for(int i = 0; i < mcuY; i++) {
    for(int j = 0; j < mcuX; j++) {
      quantYPtrBuf = quantYPtr + (i *2) * lineOfData * 2 + j * 128;
      code_coefficients_pass_two(quantYPtrBuf, huffCodesYDc, huffCodesSizesYDc,
          huffCodesYAc, huffCodesSizesYAc, &lastYDcValue);
      quantYPtrBuf = quantYPtr + (i * 2) * lineOfData * 2 + j * 128 + 64;
      code_coefficients_pass_two(quantYPtrBuf, huffCodesYDc, huffCodesSizesYDc,
          huffCodesYAc, huffCodesSizesYAc, &lastYDcValue);
      quantYPtrBuf = quantYPtr + (i * 2 + 1) * lineOfData * 2 + j * 128;
      code_coefficients_pass_two(quantYPtrBuf, huffCodesYDc, huffCodesSizesYDc,
          huffCodesYAc, huffCodesSizesYAc, &lastYDcValue);

      quantYPtrBuf = quantYPtr + (i * 2 + 1) * lineOfData * 2 + j * 128 + 64;
      code_coefficients_pass_two(quantYPtrBuf, huffCodesYDc, huffCodesSizesYDc,
          huffCodesYAc, huffCodesSizesYAc, &lastYDcValue);

      quantCbPtrBuf = quantCbPtr + i * lineOfData + j * 64;
      ...
      quantCrPtrBuf = quantCrPtr + i * lineOfData + j * 64;
  ...
  cv::imshow(windowName, reverseBlockCr);
  cv::waitKey(0);
#endif
