...
bool jpeg_encoder::jpg_open(int p_x_res, int p_y_res, int src_channels)
{
  ...
  m_image_x        = p_x_res; m_image_y = p_y_res; /* raw image width:m_image_x, height:m_image_y  */
  m_image_bpp      = src_channels; /* byte per pixel. */
  m_image_bpl      = m_image_x * src_channels; /* raw image byte per line */
  m_image_x_mcu    = (m_image_x + m_mcu_x - 1) & (~(m_mcu_x - 1)); /* image width that is ceiled to mcu size. */
  m_image_y_mcu    = (m_image_y + m_mcu_y - 1) & (~(m_mcu_y - 1));
  m_image_bpl_xlt  = m_image_x * m_num_components; /* bit per line. m_num_components may be equals to src_channels */
  m_image_bpl_mcu  = m_image_x_mcu * m_num_components; /* bit per line in ceiled image */
  m_mcus_per_row   = m_image_x_mcu / m_mcu_x; /* number of mcus in row. */

  if ((m_mcu_lines[0] = static_cast<uint8*>(jpge_malloc(m_image_bpl_mcu * m_mcu_y))) == NULL) return false; /* allocate ceiled image width * sizeof height mcu */
  for (int i = 1; i < m_mcu_y; i++)
    m_mcu_lines[i] = m_mcu_lines[i-1] + m_image_bpl_mcu; /* set the pointer to first pixel address on each line. */
  ...
  printf("---Quantization tables Y---\n");
  for(int i = 0; i < 8; i++) {
    for(int j = 0; j < 8; j++) {
      printf(" %d", m_quantization_tables[0][i * 8 + j]);
    }
    printf("\n");
  }
  printf("---Quantization tables CbCr---\n");
  for(int i = 0; i < 8; i++) {
    for(int j = 0; j < 8; j++) {
      printf(" %d", m_quantization_tables[1][i * 8 + j]);
    }
    printf("\n");
  }
  ...
  else /* usually, we choose this condition */
  {
    memcpy(m_huff_bits[0+0], s_dc_lum_bits, 17);    memcpy(m_huff_val [0+0], s_dc_lum_val, DC_LUM_CODES);/* m_huff_bits 0:dc_lum 1:dc_chroma 2:ac_lum 3:ac_chroma */
...
void jpeg_encoder::load_block_8_8(int x, int y, int c) /* x is subsampling index of x, y also. maybe meaning of c is component(Y:0/Cb:1/Cr:2) */
{
  uint8 *pSrc;
  sample_array_t *pDst = m_sample_array;
  x = (x * (8 * 3)) + c; /* c is component. x * (8 * 3) means shift 1block at x axis.*/
  y <<= 3; /* multiplied by 8 means shift to 1block at y axis. */
  for (int i = 0; i < 8; i++, pDst += 8)
  {
    pSrc = m_mcu_lines[y + i] + x; /* set the start pixel of line in block. -128 means centarization for 2D DCT.*/
...
void jpeg_encoder::load_block_16_8(int x, int c)
{
  ...
  for (int i = 0; i < 16; i += 2, pDst += 8)
  {
    ...
    pSrc2 = m_mcu_lines[i + 1] + x; /* the next line of pSrc1. */
  ...
  for (int i = 0; i < 16; i += 2, pDst += 8)
  {
    pSrc = m_mcu_lines[i + 0] + x;
    pSrc2 = m_mcu_lines[i + 1] + x; /* the next line of pSrc1. */

    pDst[0] = pSrc[0 * 3] - 128; pDst[1] = pSrc[2 * 3] - 128; pDst[2] = pSrc[4 * 3] - 128; pDst[3] = pSrc[6 * 3] - 128;
    pDst[4] = pSrc[8 * 3] - 128; pDst[5] = pSrc[10 * 3] - 128; pDst[6] = pSrc[12 * 3] - 128; pDst[7] = pSrc[14 * 3] - 128;
  }
...
{
  int32 *q = m_quantization_tables[component_num > 0]; /* false means 0, true means 1. */
  int16 *pDst = m_coefficient_array;
  for (int i = 0; i < 64; i++)
  {
    sample_array_t j = m_sample_array[s_zag[i]]; /* apply quantization. and zig-zag. */
...
void jpeg_encoder::put_bits(uint bits, uint len) /* uint is unsigned int (=32 bit) */
{
...
void jpeg_encoder::code_coefficients_pass_one(int component_num)
{
  ...
  int16 *src = m_coefficient_array; /* quantized values. */
...
void jpeg_encoder::code_coefficients_pass_two(int component_num)
{
  ...
  int16 *pSrc = m_coefficient_array; /* quantized values. */
  uint *codes[2]; /*  codes[0] is huffman code of dc value. codes[1] is av value. */
  uint8 *code_sizes[2];
  ...
  temp1 = temp2 = pSrc[0] - m_last_dc_val[component_num]; /* minus by last dc value. */
  m_last_dc_val[component_num] = pSrc[0]; /* change to new last dc value. */
...
{
#if 0
  printf("--before: comp:%d mcu:%d-- \n", component_num, b_num); 
  for(int i = 0; i < 8; i++) {
    for(int j = 0; j < 8; j++) {
      printf(" %d", m_sample_array[i * 8 + j]);
    }
    printf("\n");
  }
#endif
  ...
  DCT2D(m_sample_array); /* executes 2D DCT? */
  ...
  printf("zigzag--comp:%d mcu:%d-- \n", component_num, b_num++); 
  for(int i = 0; i < 8; i++) {
    for(int j = 0; j < 8; j++) {
      printf(" %d", m_coefficient_array[i * 8 + j]);
    }
    printf("\n");
  }
  if (m_pass_num == 1)
...
void jpeg_encoder::process_mcu_row()
{
  ...
  {
    for (int i = 0; i < m_mcus_per_row; i++) /* executes each mcu.*/
    {
      load_block_8_8(i * 2 + 0, 0, 0);
      code_block(0);
      load_block_8_8(i * 2 + 1, 0, 0);
      code_block(0); /*  use m_sample_array for pass the data from load_block to code_block. */
...
{
  if (m_mcu_y_ofs) /* if image height(y) == 0. this code don't have to some function. */
  {
    if (m_mcu_y_ofs < 16) // check here just to shut up static analysis
    {
      for (int i = m_mcu_y_ofs; i < m_mcu_y; i++) /*  padding lines! */
...
void jpeg_encoder::load_mcu(const void *pSrc)
{
  ...
  {
    const uint8 y = pDst[m_image_bpl_xlt - 3 + 0], cb = pDst[m_image_bpl_xlt - 3 + 1], cr = pDst[m_image_bpl_xlt - 3 + 2]; /*  set the last pixel data on line. */
    uint8 *q = m_mcu_lines[m_mcu_y_ofs] + m_image_bpl_xlt; /* set the pointer for padding!*/
    for (int i = m_image_x; i < m_image_x_mcu; i++)
    {
      *q++ = y; *q++ = cb; *q++ = cr; /*  padding! */
    }
  }

  if (++m_mcu_y_ofs == m_mcu_y) /* if now mcu line buffer gets 16 line data, executes. */
  {
    process_mcu_row(); /* if it can get color conversioned and padded mcus line, we execute next step. use m_mcu_lines */
    m_mcu_y_ofs = 0;
  }
}
...
bool compress_image_to_jpeg_file(const char *pFilename, int width, int height, int num_channels, const uint8 *pImage_data, const params &comp_params)
{
  ...
  for (uint pass_index = 0; pass_index < dst_image.get_total_passes(); pass_index++) /* usually, 1. */
  {
    for (int i = 0; i < height; i++) /* each line executes in-loop funtion.*/
    {
