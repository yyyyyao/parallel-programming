 ...
 *
 *=========================================================================*/
#ifndef __itkN4BiasFieldCorrectionImageFilterOrig_hxx
#define __itkN4BiasFieldCorrectionImageFilterOrig_hxx
...
N4BiasFieldCorrectionImageFilterOrig<TInputImage, TMaskImage, TOutputImage>
::N4BiasFieldCorrectionImageFilterOrig()
//:
...
//  m_SplineOrder( 3 )
{
  ...
  this->o_dimension = 2; //in original codes, input dimension is only 2.
  for(int i = 0; i < o_dimension; i++) {
    o_NumberOfFittingLevels[i] = 1;
    o_NumberOfControlPoints[i] = 4;
  }
  this->o_MaximumNumberOfIterations[0] = 1;
  this->o_MaximumNumberOfIterationsSize = 1;
}
...
::GenerateData()
{
  ...
  // get the label from labels image.
  ...
  for( It.GoToBegin(); !It.IsAtEnd(); ++It )
    ...
    if( ( !maskImage ||
      ...
      if( It.Get() > NumericTraits<typename InputImageType::PixelType>::Zero )
        ...
        It.Set((float)log((It.Get()))); /* generates minimum error. */
        }
      }
    }
  // end of get the label from labels image.
  // end of calculate the log of the input image.

#ifdef YAO_IMPLE
  // do same thing by yao.
  printf("-----------check log result---------------\n");
  itk::ImageScanlineConstIterator<RealImageType> it2(logInputImage, inputRegion);
  it2.GoToBegin();
  while(!it2.IsAtEnd()) {
  ...
  //do same things by yao.


  //do same things by yao.
  unsigned int maximumNumberOfLevels = 1;
  for( unsigned int d = 0; d < this->o_dimension; d++ )
    {
    if( this->o_NumberOfFittingLevels[d] > maximumNumberOfLevels )
      {
      maximumNumberOfLevels = this->o_NumberOfFittingLevels[d];
      }
    }
  if( this->o_MaximumNumberOfIterationsSize != maximumNumberOfLevels )
    {
    itkExceptionMacro(
      "Number of iteration levels is not equal to the max number of levels." );
    }

  /* maximumNumberOfLevels is max(m_NumberOfFittingLevels[]) */
  for( this->m_CurrentLevel = 0; this->m_CurrentLevel < maximumNumberOfLevels;
    ...
    /* set Max Value of float to m_CurrentConvergenceMeasurement */
    //this->m_CurrentConvergenceMeasurement = NumericTraits<RealType>::max();
    this->m_CurrentConvergenceMeasurement = FLT_MAX;
    /* check num of Iteration and Convergence. */
    // Iterate until convergence or iterative exhaustion.
    while( this->m_ElapsedIterations++ <
           this->o_MaximumNumberOfIterations[this->m_CurrentLevel] &&
      ...
      /* logUncorrectedImage - logSharpenedImage */
      ...
      typename SubtracterType::Pointer subtracter_ = SubtracterType::New();
      subtracter_->SetInput1( logBiasField);
      subtracter_->SetInput2( newLogBiasField);
      subtracter_->Update();
      ...
      const MaskImageType * maskImage_ = this->GetMaskImage();
      const RealImageType * confidenceImage_ = this->GetConfidenceImage();

      ImageRegionConstIteratorWithIndex<RealImageType> It_(
        subtracter_->GetOutput(),
        subtracter_->GetOutput()->GetLargestPossibleRegion() );

      for( It_.GoToBegin(); !It_.IsAtEnd(); ++It_ )
        {
        if( ( !maskImage_ ||
              maskImage_->GetPixel( It_.GetIndex() ) == this->m_MaskLabel )
            && ( !confidenceImage_ ||
                 confidenceImage_->GetPixel( It_.GetIndex() ) > 0.0 ) )
          {
          RealType pixel = exp( It_.Get() );
          N += 1.0;
          ...
          if( N > 1.0 )
            ...
            sigma = sigma + (( pixel - mu ) * ( pixel - mu ) * ( N - 1.0 )) / N;
            }
          mu = mu * ( 1.0 - 1.0 / N ) + pixel / N;
          }
        }
      sigma = sqrt( sigma / ( N - 1.0 ) );

      //return ( sigma / mu );

      //----------------------------------------
      this->m_CurrentConvergenceMeasurement = sigma / mu;
    ...
    for( unsigned int d = 0; d < ImageDimension; d++ )
      ...
      if( this->o_NumberOfFittingLevels[d] + 1 >= this->m_CurrentLevel &&
...
template<class TInputImage, class TMaskImage, class TOutputImage>
typename
N4BiasFieldCorrectionImageFilterOrig<TInputImage, TMaskImage, TOutputImage>::RealImagePointer
N4BiasFieldCorrectionImageFilterOrig<TInputImage, TMaskImage, TOutputImage>
::SharpenImage( const RealImageType *unsharpenedImage ) const
{
  ...
  /* get maximum and miminum within the masked region. */
  ...
  for( ItU.GoToBegin(); !ItU.IsAtEnd(); ++ItU )
    {
    /* within the masked region, and pixel value is valid. */
    if( ( !maskImage ||
          maskImage->GetPixel( ItU.GetIndex() ) == this->m_MaskLabel )
        && ( !confidenceImage ||
             confidenceImage->GetPixel( ItU.GetIndex() ) > 0.0 ) )
      ...
      /* if value is during bin, do interpolate. */
  ...
  // for FFT, ->2^N.  just do zero-padding!!

  RealType exponent =
    ceil( log( static_cast<RealType>( this->m_NumberOfHistogramBins ) ) /
              log( 2.0 ) ) + 1;
  unsigned int paddedHistogramSize = static_cast<unsigned int>(
    pow( static_cast<RealType>( 2.0 ), exponent ) + 0.5 );
  ...
  // in the equation(4.7), F is gaussian.

  RealType scaledFWHM = this->m_BiasFieldFullWidthAtHalfMaximum / histogramSlope;
  RealType expFactor = 4.0 * log( 2.0 ) / vnl_math_sqr( scaledFWHM );
  RealType scaleFactor = 2.0 * sqrt( log( 2.0 )
  ...
  for( unsigned int n = 1; n <= halfSize; n++ )
    ...
    F[n] = F[paddedHistogramSize - n] = vcl_complex<RealType>( scaleFactor *
      exp( -vnl_math_sqr( static_cast<RealType>( n ) ) * expFactor ), 0.0 );
    }
  if( paddedHistogramSize % 2 == 0 )
    {
    F[halfSize] = vcl_complex<RealType>( scaleFactor * exp( 0.25 *
...
::UpdateBiasFieldEstimate( RealImageType* fieldEstimate )
{
  ...
  // space.
  // Scalar Type is Vector<RealType, 1> for B-spline smoothing filter.

  // direction cosine is just a vector.
  ...
  for ( It.GoToBegin(); !It.IsAtEnd(); ++It )
    ...
    if( ( !maskImage ||
      ...
      //store scalar & point to fieldPoints
  ...
  for( unsigned int d = 0; d < ImageDimension; d++ )
    ...
    if( !this->m_LogBiasFieldControlPointLattice )
      {
      numberOfControlPoints[d] = this->o_NumberOfControlPoints[d];
      }
    else
      {
  ...
  //first time , this condition will be executed.
  if( !this->m_LogBiasFieldControlPointLattice )
    {
...
#if 0
    itk::ImageScanlineConstIterator<BiasFieldControlPointLatticeType> it2(this->m_LogBiasFieldControlPointLattice, this->m_LogBiasFieldControlPointLattice->GetRequestedRegion());
    it2.GoToBegin();
    while(!it2.IsAtEnd()) {
      while(!it2.IsAtEndOfLine()) {
        ++it2;
...
template<class TInputImage, class TMaskImage, class TOutputImage>
typename
N4BiasFieldCorrectionImageFilterOrig<TInputImage, TMaskImage, TOutputImage>::RealType
N4BiasFieldCorrectionImageFilterOrig<TInputImage, TMaskImage, TOutputImage>
