 ...
 *
 *=========================================================================*/
#ifndef __itkN4BiasFieldCorrectionImageFilterOrig_h
#define __itkN4BiasFieldCorrectionImageFilterOrig_h
...
/**
 * \class N4BiasFieldCorrectionImageFilterOrig
...
class ITK_EXPORT N4BiasFieldCorrectionImageFilterOrig :
  public ImageToImageFilter<TInputImage, TOutputImage>
{
public:
  /** Standard class typedefs. */
  typedef N4BiasFieldCorrectionImageFilterOrig              Self;
  typedef ImageToImageFilter<TInputImage, TOutputImage> Superclass;
  typedef SmartPointer<Self>                            Pointer;
  typedef SmartPointer<const Self>                      ConstPointer;

  /** Runtime information support. */
  itkTypeMacro( N4BiasFieldCorrectionImageFilterOrig, ImageToImageFilter );
  ...
  void SetNumberOfControlPoints(unsigned int* controlPoints) {
    for(int i = 0; i < o_dimension; i++) {
      this->o_NumberOfControlPoints[i] = controlPoints[i];
    }
  };
  ...
  unsigned int GetNumberOfControlPoints() {
    return o_NumberOfControlPoints;
  }
  ...
  void SetNumberOfFittingLevels(unsigned int numIters) {
    for(int i = 0; i < o_dimension; i++) {
      this->o_NumberOfFittingLevels[i] = numIters;
    }
  };
  /**
   * Set the number of fitting levels.  One of the contributions of N4 is the
   ...
   * Default = 50.
   */
  void SetMaximumNumberOfIterations(unsigned int* numIters, unsigned int size) {
    for(int i = 0; i < size; i++) {
      this->o_MaximumNumberOfIterations[i] = numIters[i];
    }
    this->o_MaximumNumberOfIterationsSize = size;
  };
   ...
   * Default = 50.
   */
  /* this function is buggy... Yao */
  unsigned int GetMaximumNumberOfIterations() {
    return o_MaximumNumberOfIterations;
  }
...
private:
  N4BiasFieldCorrectionImageFilterOrig( const Self& ); //purposely not
                                                      // implemented
  ...
  /* yao modified. */
#define NUM_O 2
#define __MAX_ITE_SIZE 10
  unsigned int o_dimension;
  unsigned int o_MaximumNumberOfIterationsSize;
  unsigned int o_MaximumNumberOfIterations[__MAX_ITE_SIZE];
  unsigned int o_NumberOfControlPoints[NUM_O];
  unsigned int o_NumberOfFittingLevels[NUM_O];
  unsigned int o_MaskLabel;

  //BSplineScatteredDataPointSetToImageFilter's input
...
#include "itkN4BiasFieldCorrectionImageFilterOrig.hxx"
#endif
